...
```
[1] 시드 수집        Wikidata / Wikipedia(공개 라이선스) → 베이스 그래프
        ↓
[2] 뉴스 확장        뉴스에서 "사실"만 추출(원문 복제 X, 링크만 보관) → 후보 statement
        ↓
[3] 출처 등급 부여    OFFICIAL / REPORTED / RUMOR 자동 분류 + (민감 건은) 사람 검수
        ↓
[4] 그래프 적재      reified statement 형태로 저장 (출처·등급·시점 메타 포함)
        ↓
[5] 콘텐츠 생성      OFFICIAL 등급만 재료로, "재구성형"만 생성
```
...
5. **민감 카테고리는 스코프에서 제외.** 논란·사건·사생활·열애·갈등·건강·정치성향 등은 수집·저장·생성 어디에도 넣지 않는다. (한국 *사실적시 명예훼손* 리스크 — `docs/safety-guidelines.md` 참조)
6. **뉴스 원문 복제 금지.** 사실(fact)만 추출하고 표현은 가져오지 않는다. 출처는 URL 링크로만 보관.
...
- **M1** 온톨로지 스키마 확정 + 데이터 모델(타입) 구현 + validation (스키마 §5)
- **M2** Wikidata 시드 수집기(SPARQL, 레퍼런스 달린 OFFICIAL만) → 베이스 그래프 적재
- **M6a** 읽기전용 query API(엔티티 검색·관계·연표·등급/기간 필터) + 최소 탐색 UI
